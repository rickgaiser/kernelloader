...
int CDDA_Init(void)
{
	...
	if (!s_ClientInit.server)
		...
		if ((retVal = SIF_BindRPC(&s_ClientInit, CD_SERVER_INIT)) &&
			...
			retVal =
				SifCallRpc(&s_ClientInit, 0, 0, &s_InitMode, 4, 0, 0, 0,
				0) >= 0;
...
int CDDA_ReadTOC(CDDA_TOC * apTOC)
{
	...
	if (SifCallRpc(&s_ClientNCmd, CD_CMD_GETTOC, 0, s_GetTOCmd, 12,
			s_NCmdRecvBuff, 8, _cd_callback, 0) >= 0) {
...
DiskType CDDA_DiskType(void)
{
	...
	if (SifCallRpc(&s_ClientSCmd, CD_CMD_GETDISCTYPE, 0, 0, 0, s_SCmdRecvBuff,
			4, 0, 0) >= 0) {
...
void CDDA_Standby(void)
{
	...
	if (SifCallRpc(&s_ClientNCmd, CD_CMD_STANDBY, SIF_RPC_M_NOWAIT,
			0, 0, 0, 0, _cd_callback, NULL) < 0)
		s_SyncFlag = 0;
...
void CDDA_Pause(void)
{
	...
	if (SifCallRpc(&s_ClientNCmd, CD_CMD_PAUSE, SIF_RPC_M_NOWAIT,
			0, 0, 0, 0, _cd_callback, NULL) < 0)
		s_SyncFlag = 0;
...
int CDDA_Stop(void)
{
	...
	if (SifCallRpc(&s_ClientNCmd, CD_CMD_STOP, SIF_RPC_M_NOWAIT, 0, 0, 0, 0,
			_cd_callback, NULL) >= 0)
...
int CDDA_SetMediaMode(MediaMode aMode)
{
	...
	*(unsigned int *) s_SCmdRecvBuff = (unsigned int) aMode;
	...
	if (SifCallRpc(&s_ClientSCmd, CD_CMD_MMODE, 0, s_SCmdRecvBuff, 4,
			s_SCmdRecvBuff, 4, 0, 0) >= 0)
...
void CDDA_DiskReady(void)
{
	...
	SifCallRpc(&s_ClientDiskReady, 0, 0, &s_DiskReady, 4, &s_DiskReady, 4, 0,
		0);
...
int CDDA_RawRead(int aStartSec, int aCount, unsigned char *apBuf)
{
	...
	if (SifCallRpc(&s_ClientNCmd, CD_CMD_CDDAREAD,
			SIF_RPC_M_NOWAIT | SIF_RPC_M_NOWBDC, s_ReadData, 24, NULL, 0,
			(void *) _cd_callback, s_ReadResp) >= 0)

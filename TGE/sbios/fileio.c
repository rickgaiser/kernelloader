...
int fioInit()
{
	...
	res = SifBindRpc(&_fio_cd, 0x80000001, SIF_RPC_M_NOWAIT, fioInitStage2, NULL);
	if (res < 0) {
		printf("fioInit() failed\n");
		return res;
	}
...
#define WaitSema(sema) simulateWaitSema(&sema)

/** Only simulated, this is not good without linux scheduling support. */
void simulateWaitSema(volatile u32 *sema)
{
	u32 status;
